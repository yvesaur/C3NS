...
class Parser:
	...
	def parse(self):
		...
		if not res.error and self.current_tok.type != TT_EOF:
			return res.failure(InvalidSyntaxError(
				...
				"Expected '+', '-', '*', '/', '^', '==', '!=', '<', '>', <=', '>=', 'AND' or 'OR' ->" + str(self.current_tok.type) + " " + str(self.current_tok.value)
			))
		return res
	...
	def expr(self):
		...
		if self.current_tok.matches(TT_KEYWORD, 'INT') or self.current_tok.matches(TT_KEYWORD, 'FLT') or self.current_tok.matches(TT_KEYWORD, 'LFLT') or self.current_tok.matches(TT_KEYWORD, 'CHR') or self.current_tok.matches(TT_KEYWORD, 'STR') or self.current_tok.matches(TT_KEYWORD, 'BINAL'):
			res.register_advancement()
			self.advance()
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected DATA TYPE, 'IF', 'FOR', 'WHILE', 'FUNC' int, float, identifier, '+', '-', '(', '[' or 'NOT'"
			))
	...
	def if_expr_c(self):
		...
		if self.current_tok.matches(TT_KEYWORD, 'ELSE'):
			...
			if self.current_tok.type == TT_NEWLINE:
				...
				if self.current_tok.matches(TT_RBRACKET, None):
					res.register_advancement()
					self.advance()
				else:
					return res.failure(InvalidSyntaxError(
	...
	def for_expr(self):
		...
		if not self.current_tok.matches(TT_SEMICOLON, None):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected Semicolon ;"
			))
		res.register_advancement()
		self.advance()
	...
	def func_def(self):
		...
		if self.current_tok.type == TT_IDENTIFIER:
			...
			if self.current_tok.type != TT_LPAREN:
				return res.failure(InvalidSyntaxError(
					self.current_tok.pos_start, self.current_tok.pos_end,
					f"Expected Identifier or Left Parenthesis"
				))
		...
		if self.current_tok.type == TT_IDENTIFIER:
			...
			if self.current_tok.type != TT_RPAREN:
				return res.failure(InvalidSyntaxError(
					self.current_tok.pos_start, self.current_tok.pos_end,
					f"Expected Identifier or Right Parenthesis"
				))
...
class BuiltInFunction(BaseFunction):
  ...
  #####################################
	...
	execute_load.arg_names = []

	def execute_load_int(self, exec_ctx):
		while True:
			text = input()
			try:
				number = int(text)
...
BuiltInFunction.load_int   = BuiltInFunction("load_int")
...
global_symbol_table.set("SHOW_RET", BuiltInFunction.show_ret)
global_symbol_table.set("LOAD", BuiltInFunction.load)
global_symbol_table.set("LOAD_INT", BuiltInFunction.load_int)
