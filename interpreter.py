...
class Lexer:
	...
	def make_ident(self):
		...
		tok_type = TT_KEYWORD if id_str in DATA_TYPE else TT_IDENT
...
class Parser:
	...
	def expr(self):
		...
		if self.curr_tok.matches(TT_KEYWORD, 'INT') or self.curr_tok.matches(TT_KEYWORD, 'CHR') or self.curr_tok.matches(TT_KEYWORD, 'FLT') or self.curr_tok.matches(TT_KEYWORD, 'LFLT') or self.curr_tok.matches(TT_KEYWORD, 'BINAL') or self.curr_tok.matches(TT_KEYWORD, 'STR'):
			res.register_advancement()
			self.advance()

			if self.curr_tok.type != TT_IDENT:
				return res.failure(InvalidSyntaxError(
					self.curr_tok.pos_start, self.curr_tok.pos_end,
					"Expected identifier"
				))
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.curr_tok.pos_start, self.curr_tok.pos_end,
				"Expected Data type, int, float, identifier, '+', '-' or '('"
			))
